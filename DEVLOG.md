...
## 0.0.0.14 — Установка моделей Ollama по имени (ollama pull)

- **Модели Ollama:**
  - На сайте Ollama нет прямой ссылки на скачивание — только команда вида `ollama run llama3.2`. В настройках добавлен **основной способ**: поле «Имя модели для Ollama» (например, llama3.2, mistral, qwen2:7b) и кнопка **«Установить модель (ollama pull)»** — загрузка через API Ollama, после успеха модель автоматически выбирается для чата.
  - Блок «Загрузка по прямой ссылке на файл» оставлен для случаев, когда есть прямая ссылка на .zip/.gguf/.bin. Кнопка «Каталог имён моделей Ollama» открывает ollama.com/library с подсказкой использовать имя из команды run.
- **Прогресс установки:**
  - Во время pull отображаются спиннер, текст «Идёт загрузка: …» с текущим статусом от Ollama и подпись «Идёт загрузка — может занять несколько минут. Не закрывайте окно.» При появлении данных по слоям показывается процент и заполняется прогресс-бар.
  - Исправлена обработка ответов API (await для AsyncClient.pull, защита от None в total/completed), чтобы не было ошибок и зависаний интерфейса.

*Строк: 6 487 | Символов: 234 447*
